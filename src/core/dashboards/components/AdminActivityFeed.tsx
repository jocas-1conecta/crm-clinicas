                    ...
                    <div key={clinic.id} className="flex items-start space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                                {new Date(clinic.created_at).toLocaleDateString('es-PE', { day: '2-digit', month: 'short', year: 'numeric' })}
                                {' • '}{clinic.status === 'activa' ? '✅ Activa' : clinic.status === 'pendiente' ? '⏳ Pendiente' : '⏸️ Suspendida'}
                            </p>
                        </div>
                    </div>
                ))}
